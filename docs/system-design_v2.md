...
|---|---|---|
| `GET /catalog` | none | List/filter coins by country, denomination, coin name (text match), year range — paginated (`page`, `limit`). Returns `status = 'approved'` coins only (§4.7) |
...
**Getting a pending coin into a set, concretely.** The only existing UI for adding a coin to a set is the "Add coins" panel on `/sets/[id]` (§3.1), and it searches via `GET /catalog` — which, after the filter above, will never surface the caller's own pending coin. Rather than teach that endpoint an optional-auth "and also show my own pending coins" mode (a pattern nothing else in this API uses — every route today is either `@Public()` with no user context at all, or fully guarded, §4.2), the submission flow's own confirmation step is the add mechanism: `POST /catalog`'s response already has the new coin's id, so the confirmation screen lists the caller's own sets (`GET /sets`, already built) and lets them add the new coin to one directly via the existing `PATCH /sets/:id/coins` — no catalog search involved for this one coin. A caller with no sets yet gets a lightweight inline "create a set and add this coin" option instead (`POST /sets` immediately followed by the same `PATCH /sets/:id/coins` call), rather than being routed to the general `/sets/new` page, which has no way to carry a preselected coin id through — that would just relocate the same "how do I find my own pending coin" problem one page later. This is a narrower, purpose-built path, not a general "browse my pending submissions" feature.

**Accepted gap: no recovery path if the confirmation screen is skipped.** If a caller navigates away from the submission confirmation without adding the new coin to a set, there is no "my submissions" list anywhere to find it again — it stays reachable only by an id the caller no longer has, until it's approved and surfaces in normal `GET /catalog` browse. Deliberately not solved now: the coin itself is never lost (it's already in the shared table, still reviewable, still becomes generally discoverable once approved), so the cost of skipping the confirmation screen is a delay for that one user, not data loss. Revisit with a "my submissions" view only if this friction actually shows up in practice (§7) — it's a bigger surface (a new authenticated list endpoint plus a new page) than this feature otherwise needs.
...
- **"My submissions" view:** if skipping the submission confirmation screen and losing track of a pending coin (§4.7) turns out to be a real recurring complaint rather than a theoretical edge case, add an authenticated `GET /catalog?submittedByMe=true` (or a dedicated list endpoint) plus a small page listing the caller's own submissions regardless of status. Not built now because it's a second list surface for a problem that costs a delay, not data loss (§4.7).