...
| Catalog contribution review | Manual, direct-to-DB status flip — no admin role, endpoint, or UI in v1 (§4.7) |
...
|---|---|---|
| `GET /catalog` | none, optional | List/filter coins by country, denomination, coin name (text match), year range — paginated (`page`, `limit`). Returns `status = 'approved'` coins only, unless `submittedByMe=true` is sent with a valid bearer token, in which case it returns the caller's own submissions across every status instead (§4.7) |
| `GET /catalog/:id` | none | Single coin detail — not status-filtered; reachable by id regardless of review status (§4.7) |
| `POST /catalog` | required | Submit a coin missing from the catalog (PRD Requirement 19) — creates it with `status: 'pending'`, `submittedByUserId: <caller>`; 409 on a natural-key match against any existing status (§4.7) |
...
coins
  id (pk), country, denomination, year, mint_mark (NOT NULL, default ''), variety (NOT NULL, default ''),
  ...
  status (CoinStatus enum: 'approved' | 'pending' | 'rejected', NOT NULL, default 'approved' — §4.7),
  submitted_by_user_id (FK → users.id, nullable, onDelete: SetNull — §4.7),
  submitted_at (nullable, set when submitted_by_user_id is — §4.7),
...
Notes:
- **No `numista_type_id` column (removed 2026-07-21).** An earlier draft of this doc carried a nullable, non-unique `numista_type_id` enrichment column as a placeholder for a possible future Numista backfill. Decided to drop it instead of carrying an always-`null` column no v1 code ever writes or reads — Numista remains optional/permission-gated future enrichment (§4.3), and adding one nullable column back is a trivial additive migration if that's ever pursued, so there's no reason to pay the schema-clutter cost now. Per-coin uniqueness lives on the composite `(country, denomination, year, mint_mark, variety)` — the same natural key the import script already dedupes on (§4.3), backed by a real DB constraint. Note for whoever revisits this: putting a future `numista_type_id` into that composite would be a bug, not just unnecessary — a single Numista "type" spans many mint years, so uniqueness can never live on that column alone, and any coin without one would have `NULL` there, which Postgres doesn't treat as equal in a unique constraint, silently defeating the constraint for every such row.
...
- **`status`/`submitted_by_user_id`/`submitted_at` (added 2026-07-25 for PRD Requirement 19, user-submitted coins — full endpoint/review design in §4.7).** `status` defaults to `'approved'`, so the column is purely additive against the existing imported catalog — every pre-existing row is `'approved'` with no backfill needed; only a row created via the new `POST /catalog` endpoint ever starts at `'pending'`. The `@@unique([country, denomination, year, mintMark, variety])` constraint above is unchanged and still applies across every status value, not just `'approved'` — a `rejected` coin still occupies its natural key, which is deliberate (§4.7): a rejected submission is never deleted, so resubmitting the same natural key isn't possible, and the fix for a bad rejection is flipping that row's status back, not creating a second row beside it. `submitted_by_user_id` is nullable (`null` for every import-script row, same as always) and `onDelete: SetNull`, matching the same defensive-FK convention already used for `cloned_from_canonical_id`/`cloned_from_user_set_id` above — losing the submitter attribution if their account is ever removed is acceptable; silently cascading the coin itself away is not, especially once other users may have added it to their own sets.
...
**v1 source (primary):** open data — Wikipedia mintage lists / US Mint publications for U.S. coin data, Wikimedia Commons for images. License/ToS confirmed directly (backlog task 0.2, see [docs/catalog-data-licensing.md](../docs/catalog-data-licensing.md) for the full check): no API key or hard rate limit, but the Wikimedia API does require a descriptive `User-Agent` with contact info per its API:Etiquette policy (incorporated into the Wikimedia Terms of Use by reference) — the import script must set one, not skip it. usmint.gov itself blocks automated fetches outright (403 on every policy/terms URL tried) — it's a manual cross-reference source only, same as v1 already treated it, never something the script fetches from programmatically.

Image licensing needs per-image handling, not a blanket assumption: U.S. Mint-credited images on Commons are typically public domain, but Commons also hosts user-photographed coin images under CC BY-SA and similar share-alike licenses that carry an attribution obligation — and usmint.gov's own copyright page is explicit that *nothing* on it should be assumed public domain by default, so "US Mint-sourced" alone isn't a safe shortcut either. Two safeguards, both cheap: (1) the import script only pulls images where the Commons `imageinfo` API's `extmetadata.Copyrighted` is `"False"` or `extmetadata.AttributionRequired` is `"false"` — confirmed live against real Commons files (docs/catalog-data-licensing.md §2) — and records `image_source`/`image_license` (from `extmetadata.Credit`/`LicenseShortName`) per coin row rather than assuming one blanket license for everything imported; (2) regardless, add a one-line attribution note in the app footer covering Wikipedia-derived catalog data generally (CC BY-SA), since that's a one-sentence cost that removes the question entirely rather than requiring the import script to get every edge case right. Coin *design* copyright (separate from photo copyright) is also settled for the current scope: any US coin published before March 1, 1989 is public domain by rule (US coins have never carried a copyright notice) — Lincoln Wheat Cents (1909–1958) and Morgan Dollars (1878–1921) both clear this cleanly; a future State Quarters template (1999–2008) would not, and needs its own per-design employee-vs.-contractor check before import, per docs/catalog-data-licensing.md §3.
...
### 4.7 Catalog contribution & review (user-submitted coins)

Added 2026-07-25, after v1's core loop shipped. Logged-in users can add a coin the catalog is missing (PRD Requirement 19) — the catalog import script (§4.3) only ever covers what someone thought to seed ahead of time, and a real collector will eventually own something it doesn't have yet.

**Submission.** `POST /catalog` (auth required) takes every field `coins` actually requires to create a row, not just the natural-key ones: country, denomination, year, mint mark, variety, **and `name`** — `coins.name` is `NOT NULL` with no default (§4.1) and drives the catalog search form (§3.1), so a submission missing it isn't just incomplete, it fails to create at all. Mint-mark and variety run through the identical sanitize-to-`''` normalization the offline import script already applies (§4.3), so `"None"`/`"N/A"`/whitespace/`null` all collapse to the one representation the natural key actually dedupes on; country/denomination/name are trimmed and the dedup check below (not the DB constraint, which stays case-sensitive) treats country/denomination case-insensitively, so `"usa"` submitted against an existing `"USA"` is caught as a duplicate rather than silently creating a visually-inconsistent second entry. The normalization function (`sanitizeIdentityField`, currently `scripts/import-catalog/lib/sanitize.ts`) should move into `packages/shared` so the import script and this endpoint share one implementation instead of two copies that can silently drift apart — exactly the class of bug §4.1's import notes already warn about, just with a second writer now. The created row gets `status: 'pending'`, `submitted_by_user_id: <caller>`, `submitted_at: now()`.

A submission form only realistically needs a free-text country/denomination the first time either value is ever introduced to the catalog — every other submission is filling a gap in an already-seeded series (a missing year/mint/variety of an existing country+denomination), where a picker populated from the catalog's own distinct existing values would prevent most casing/spelling drift before it happens. Worth building once there's a cheap way to query distinct values, but not required for this to ship correctly — the case-insensitive dedup check above is what actually prevents a duplicate from existing, a picker just prevents the *cosmetic* inconsistency of two visually-different strings both being "correct."

**Dedup, checked twice on purpose.** The service looks for an existing row matching the normalized natural key across *every* status (`approved`, `pending`, `rejected`) and returns 409 if one exists; the DB's own `@@unique([country, denomination, year, mintMark, variety])` constraint (§4.1, unchanged) is the backstop that catches anything the app-level check races past. A coin already in the catalog under any status blocks a new submission of the same natural key — including a `rejected` one, since resubmitting doesn't create a second row, correcting the existing row's status does (§4.1).

**Discoverability vs. access.** `status` is a 3-value enum (`approved` | `pending` | `rejected`), default `'approved'` (§4.1) — every pre-existing/imported row is `'approved'` by construction. `GET /catalog` filters to `status = 'approved'` by default; that's the only thing review actually gates. `GET /catalog/:id` is **not** status-filtered — a coin is reachable by direct id regardless of status, the same way a `user_set`/`canonical_set`'s coins are reachable without a separate per-coin visibility check (§4.6). Practically, this means a `pending` coin already added to a set is visible to anyone who can see that set (all sets, by default — §4.6): approval controls catalog *browse/search* discoverability, not access via a known id, consistent with how visibility already works everywhere else in this app. `status`/`submitted_at` are part of the shared `CatalogCoin` shape returned by both endpoints (so the web app can render a "pending review" badge on a coin's detail page, §3.1); `submitted_by_user_id` deliberately is **not** — `GET /catalog/:id` is unauthenticated and reachable by anyone, so putting a raw user id on a publicly-readable payload would leak submitter identity for no product benefit. The submitter doesn't need it echoed back either: `POST /catalog`'s own response, returned once at submission time, is enough for the frontend to know "I submitted this."

**The submitter isn't blocked on review.** `user_set_coins` and `ownership` join to `coins` by id only and have never filtered on any coin attribute (§4.1), so the id `POST /catalog` returns can be used immediately in `PATCH /sets/:id/coins` and `PATCH /collection/:coinId`, exactly like any other catalog coin. Review only ever changes whether *other* users can find it via `GET /catalog`.

**Getting a pending coin into a set, concretely.** The only existing UI for adding a coin to a set is the "Add coins" panel on `/sets/[id]` (§3.1), and it searches via `GET /catalog` — which, after the filter above, will never surface the caller's own pending coin. Rather than teach that endpoint an optional-auth "and also show my own pending coins" mode (a pattern nothing else in this API used at the time — every route was either `@Public()` with no user context at all, or fully guarded, §4.2; this was later revisited for a different purpose, see "Recovery path" below), the submission flow's own confirmation step is the add mechanism: `POST /catalog`'s response already has the new coin's id, so the confirmation screen lists the caller's own sets (`GET /sets`, already built) and lets them add the new coin to one directly via the existing `PATCH /sets/:id/coins` — no catalog search involved for this one coin. A caller with no sets yet gets a lightweight inline "create a set and add this coin" option instead (`POST /sets` immediately followed by the same `PATCH /sets/:id/coins` call), rather than being routed to the general `/sets/new` page, which has no way to carry a preselected coin id through — that would just relocate the same "how do I find my own pending coin" problem one page later. This is a narrower, purpose-built path, not a general "browse my pending submissions" feature.

**Recovery path if the confirmation screen is skipped (added 2026-07-31, `docs/backlog_my-submissions.md`).** The gap described in the original version of this section — a caller navigating away from the submission confirmation had no way back to a pending coin, since `GET /catalog` was `status: 'approved'`-only and there was no other authenticated list — turned out to be worth closing rather than a theoretical edge case. Built as `submittedByMe=true` on the *existing* `GET /catalog` (§3), gated behind a new **optional-auth** pattern rather than a second endpoint: `OptionalJwtAuthGuard` (`apps/api/src/auth/guards/optional-jwt-auth.guard.ts`) overrides Passport's `handleRequest` to resolve `undefined` instead of throwing on a missing/invalid/expired token, so `GET /catalog` — still `@Public()` — can now populate `req.user` when a valid token *is* present without rejecting anonymous callers. `submittedByMe=true` is only honored when a caller is actually authenticated; sent anonymously, it's silently ignored and the endpoint behaves exactly as before. When honored, it *replaces* the `status: 'approved'` filter with `submittedByUserId: <caller>` across every status, rather than OR-ing into the normal browse results — matching this section's original framing ("a small page listing the caller's own submissions regardless of status") precisely. `apps/web/src/app/catalog/mine/page.tsx` is that page, reachable from `SiteNav`'s authenticated link group; the frontend still doesn't need `submitted_by_user_id` echoed back (the reasoning two paragraphs up is unchanged) since it already knows which coins are "mine" from having asked for them by `submittedByMe=true` in the first place. This is purely a *recovery* path — the submission-confirmation flow described in "Getting a pending coin into a set" above is still the primary add mechanism and is unchanged.

**Review, v1: manual, direct-to-database, no admin role or endpoint.** There's realistically one reviewer (Andrew) and a handful of submissions — building `@Admin()` guards, an approval endpoint, or a review UI for a queue that size is pure overhead the app doesn't need yet, the same reasoning already applied to deferring `is_public` above and to skipping rate limiting (§5). Approving or rejecting a pending coin is a direct `UPDATE coins SET status = ... WHERE id = ...` via Prisma Studio or a throwaway script — the same "manual, direct DB" pattern already used for every cleanup/verification pass logged in `CLAUDE.md`. Rejected rows are never deleted — nothing in this schema has ever deleted a `coins` row, and this doesn't start (there's no `onDelete` path defined for it, §4.1) — a rejected coin just stays permanently excluded from `GET /catalog`, and a bad rejection is fixed by flipping the status back, not by recreating the row.
...
| Catalog contribution review | Manual, direct-to-DB status flip | Admin API / full admin review UI | One realistic reviewer, a handful of expected submissions — an approval endpoint or UI is schema/build cost with no v1 payoff, same reasoning already applied to deferring `is_public` above; revisit if submission volume or reviewer count ever grows beyond what direct DB access comfortably handles (§4.7) |
...
- **Catalog contribution review:** if submission volume (or the number of people who'd need reviewer access) ever outgrows what's comfortable to handle directly in Prisma Studio, add a minimal `@Admin()`-guarded list/approve/reject API (§4.7) before reaching for a full review UI — the same incremental-build bias already applied to `is_public` and set-editor reordering above.
- **"My submissions" view:** built 2026-07-31 (§4.7, `docs/backlog_my-submissions.md`) — `GET /catalog?submittedByMe=true` plus `apps/web/src/app/catalog/mine/page.tsx`. No longer deferred.