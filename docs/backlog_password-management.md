...
- [x] 3.1 `apps/api/prisma/schema.prisma`: add a `PasswordResetToken` model (`id`, `userId` FK → `User.id` `onDelete: Cascade`, `tokenHash` unique, `expiresAt`, `usedAt DateTime?`, `createdAt`) per decision 8. Generate + apply a migration.
...
- [x] 3.3 `POST /auth/forgot-password` — `@Public()`, `@Throttle({ default: { limit: 3, ttl: 3_600_000 } })` (decision 7). `ForgotPasswordDto` (`email`). Always returns 200 with an identical generic message regardless of whether the email exists (prevents user enumeration) — internally, if a user is found, generates a token, stores its hash + a short expiry (e.g. 1 hour) in `PasswordResetToken`, and sends the reset email (3.2).
- [x] 3.4 `POST /auth/reset-password` — `@Public()`. `ResetPasswordDto` (`token`, `newPassword`). Looks up `PasswordResetToken` by hash, rejects (400) if not found/expired/already used, otherwise updates the user's password hash, marks the token `usedAt`, and calls `TokenService.revokeAllForUser` (available since Step 2 — no retrofit needed here, unlike Step 1's change-password).
- [x] 3.5 Unit tests: `forgotPassword` returns the same response shape whether or not the email exists, and only calls the email-send method when it does; `resetPassword` rejects an expired token, an already-used token, and a wrong token, and accepts a valid one, calling `revokeAllForUser` only on success.
- [x] 3.6 `apps/web/src/lib/auth-api.ts`: add `forgotPassword(email)` and `resetPassword(token, newPassword)`.
- [x] 3.7 New `apps/web/src/app/forgot-password/page.tsx` — public, an email-only form calling 3.6's `forgotPassword`; on submit always shows the same generic "if that email exists, a reset link was sent" success message regardless of the API's (also-generic, by design 3.3) response.
- [x] 3.8 New `apps/web/src/app/reset-password/page.tsx` — public, reads `token` from `useSearchParams()`, a new-password form calling 3.6's `resetPassword`; on success, redirect to `/login` (optionally with a query flag the login page can use to show a "password updated, log in again" message).
- [x] 3.9 `apps/web/src/app/login/page.tsx`: add a "Forgot password?" link to `/forgot-password` near the password field.
- [x] 3.10 `apps/web/src/lib/i18n/locales/en.ts` and `es.ts`: add `forgotPassword.*`, `resetPassword.*`, `login.forgotPasswordLink` keys to both files.
- [x] 3.11 Component tests for the two new forms, following the existing form-test convention — happy path plus at least one error case (expired/invalid reset token) each.
...
- 3.1: the migration SQL (`20260923200000_add_password_reset_token`) was written by hand to match `prisma migrate diff` output, same pattern as earlier migrations, because the build machine couldn't reach Neon. **It has not been applied yet** — run `prisma migrate deploy` before deploying (Render's build doesn't run migrations).
- 3.2: calls Resend's REST API with the built-in `fetch` instead of adding the `resend` SDK (one POST, no new dependency). Env vars: `RESEND_API_KEY` (unset = no email sent, request still returns the normal response), `EMAIL_FROM` (defaults to `onboarding@resend.dev`), `FRONTEND_URL` (falls back to `CORS_ORIGIN`, then `http://localhost:3000`), and `EMAIL_LOG_RESET_LINKS=true` to log links locally when there's no Resend key.
- 3.3–3.4: logic lives in a new `PasswordResetService` rather than `AuthService`. Beyond the spec: requesting a new link deletes earlier unused ones; the token is claimed with a conditional `updateMany` so two racing requests can't both use one link; the email send isn't awaited and a failed send is logged, not returned, so neither response time nor errors reveal whether the email exists; `/auth/reset-password` has the same 5/min throttle as login.
- 3.8: after a successful reset the page also clears any access token stored in the browser, and `/login?reset=success` shows a "password was reset" notice.
- 3.12 and the `test:e2e` part of 3.13 need the real Neon DB and Resend, so they're still to do.
